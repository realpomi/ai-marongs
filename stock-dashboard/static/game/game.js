...
canvas.addEventListener('touchstart', (e) => {
    ...
    // 공 근처 터치 확인
    const ballPos = state.currentBall.position;
    const dist = Math.hypot(coords.x - ballPos.x, coords.y - ballPos.y);

    if (dist < CONFIG.ballRadius * 4) {
        state.isDragging = true;
        dragStart = coords;
    }
}, { passive: false });
...
window.addEventListener('touchmove', (e) => {
    if (!state.isDragging || state.isLaunched) return;
    e.preventDefault();
    ...
    // 드래그 거리 제한
    const dx = state.launchPos.x - coords.x;
    const dy = state.launchPos.y - coords.y;
    const dist = Math.hypot(dx, dy);
    ...
    if (dist > CONFIG.maxPullDistance) {
        ...
        state.aimPos.x = coords.x;
        state.aimPos.y = coords.y;
    }
}, { passive: false });
